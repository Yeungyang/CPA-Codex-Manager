        ...
        *,
        ...
        try:
            ...
            try:
                ...
                return OAuthCompletionResult(success=False, error_message=f"OAuth 登录密码响应解析失败: {exc}")
            ...
            if page_type == "email_otp_verification" or "email-verification" in continue_url:
                if email_adapter is None:
                    return OAuthCompletionResult(success=False, error_message="OAuth 登录触发二次邮箱验证，但当前没有可用邮箱适配器")
                self._log("[OAuth补全] 触发二次邮箱验证，正在等待新验证码...")
                otp_code = email_adapter.wait_for_verification_code(email, timeout=60)
                if not otp_code:
                    return OAuthCompletionResult(success=False, error_message="OAuth 登录未收到二次邮箱验证码")
                ...
                try:
                    ...
                    return OAuthCompletionResult(success=False, error_message=f"OAuth 二次邮箱验证响应解析失败: {exc}")
            ...
            if not consent_url:
                return OAuthCompletionResult(success=False, error_message="OAuth 登录后未获得 consent URL")
            ...
            return OAuthCompletionResult(success=False, error_message=f"OAuth 补全异常: {exc}")
